...
export const listActiveChallengeAccounts = async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const accounts = await prisma.cTraderAccount.findMany({
      where: {
        status: { in: ['active', 'assigned', 'assigned_pending_access', 'passed', 'awaiting_reset', 'withdraw_requested', 'admin_checking'] },
  ...
  try {
    ...
    res.json({
      account: {
        ...
        mt5_login: account.mt5Login ?? account.accountNumber,
        mt5_password: account.mt5Password ?? null,
        mt5_server: account.mt5Server ?? account.brokerName ?? null,
        platform: account.platform ?? 'ctrader',
...
export const getDashboardStats = async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [
      ...
      prisma.cTraderAccount.count({ where: { status: { in: ['active', 'funded', 'assigned', 'assigned_pending_access', 'passed', 'awaiting_reset', 'withdraw_requested'] } } }),
